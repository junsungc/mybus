...
const Map = () => {
    ...
    const setBusMarker = () => {
        if (busLocation) {
            ...
            busLocation.forEach(e => {
                ...
                var iwContent = `<div class ="label"><span class="left"></span><span class="center">${e.stationSeq}정류소 ${e.remainSeatCnt}석</span><span class="right"></span></div>` // 인포윈도우에 표출될 내용으로 HTML 문자열이나 document element가 가능합니다
                var iwPosition = new kakao.maps.LatLng(e.latitude, e.longitude); //인포윈도우 표시 위치입니다
            ...
            showMarker(currentMap, markers, infoWindows);
        }
    }

    const showMarker = (map, markers, infoWindows) => {
        for (var i = 0; i < markers.length; i++) {
            markers[i].setMap(map);
            infoWindows[i].setMap(map);
            setTimeout((markers, infoWindows) => {
                for (var i = 0; i < markers.length; i++) {
                    markers[i].setMap(null);
                    infoWindows[i].setMap(null);
                }
            }, 5000, markers, infoWindows);
        }
    }
    ...
    return (
        <>
            ...
            <hr />
            <button onClick={() => showMarker(null)}>클릭</button>
        </>
    )
}
